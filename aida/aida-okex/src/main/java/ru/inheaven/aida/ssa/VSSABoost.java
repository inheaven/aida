...
public class VSSABoost {
    ...
    private static final int MAX_VSSA_ITERATION = 1000000;
        ...
        for (int p = 0; p < Runtime.getRuntime().availableProcessors()-1; ++p){
            futures.add(executor.submit(() -> {
                for (int v = 0; v < MAX_VSSA_ITERATION; v++){
                    try {
                        boost(series, localQueue, fitQueue);
                    } catch (Exception e) {
    ...
    private void boost(List<Double> series, Queue<VSSA> localQueue, Queue<VSSA> fitQueue) {
        int N;
