...
@Singleton
public class SimpleBittrexEthereum extends SimpleBittrexAbstract{
    private Logger log = LoggerFactory.getLogger(SimpleBittrexEthereum.class);
...
//            "3d775f67d7f5429eaa91a59f5f1ae114");
    ...
    @Inject
    public SimpleBittrexEthereum(BittrexMarket bittrexMarket) {
        super(bittrexMarket);

        scheduler(updateOpenOrders(), 0, 1, SECONDS);

        start(new CurrencyPair("BCH", "ETH"));
        start(new CurrencyPair("LTC", "ETH"));
//        start(new CurrencyPair("NXT", "ETH"));
        start(new CurrencyPair("DASH", "ETH"));

        start(new CurrencyPair("ETC", "ETH"));
//        start(new CurrencyPair("EXP", "ETH"));
        start(new CurrencyPair("XRP", "ETH"));
        start(new CurrencyPair("NEO", "ETH"));
//        start(new CurrencyPair("GRC", "ETH"));
        start(new CurrencyPair("XMR", "ETH"));
        start(new CurrencyPair("ADA", "ETH"));
        start(new CurrencyPair("TRX", "ETH"));
        start(new CurrencyPair("GNT", "ETH"));
    }
    ...
    private BigDecimal getShare(CurrencyPair currencyPair){
        return getBittrexMarket().getShare().divide(getBittrexMarket().getTicker(CurrencyPair.ETH_BTC).getLast(), 8, HALF_EVEN);
    }
...
//                        : 0;
                ...
                BigDecimal amount = new BigDecimal(0.007/price.doubleValue());
    ...
    void trade(CurrencyPair currencyPair, BigDecimal price, BigDecimal amount, Order.OrderType orderType) {
        ...
        try {
            ...
            if (BID.equals(orderType) && amount.multiply(price).compareTo(wallet.getBalance(ETH).getAvailable()) > 0){
                return;
            }
