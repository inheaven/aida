...
public final class BittrexAdapters {
    ...
    public static AccountInfo adaptAccountInfo(List<BittrexBalance> balances) {
        ...
        for (BittrexBalance balance : balances) {
            wallets.add(new Wallet(balance.getCurrency().toUpperCase(), balance.getAvailable(), "available"));
            wallets.add(new Wallet(balance.getCurrency().toUpperCase(), balance.getBalance().subtract(balance.getAvailable()), "orders"));
        }
