...
import org.ujmp.ojalgo.calculation.*;
...
public class OjalgoDenseDoubleMatrix2D extends AbstractDenseDoubleMatrix2D implements Wrapper<MatrixStore<Double>> {
	...
	public Matrix divide(final double factor) {
        final PhysicalStore<Double> retVal = PrimitiveDenseStore.FACTORY.makeZero(this.getRowCount(), this.getColumnCount());

        retVal.fillMatching(PrimitiveDenseStore.FACTORY.function().divide().second(factor), matrix);
	...
	public double det() {
		final org.ojalgo.matrix.decomposition.LU<Double> lu = org.ojalgo.matrix.decomposition.LU.makePrimitive();
		lu.compute(matrix);
	...
	public Matrix minus(final double factor) {
		...
		retVal.fillMatching(PrimitiveDenseStore.FACTORY.function().subtract().second(factor), matrix);
	...
	public Matrix plus(final double factor) {
		...
		retVal.fillMatching(PrimitiveDenseStore.FACTORY.function().add().second(factor), matrix);
	...
	public Matrix times(final double factor) {
		...
		retVal.fillMatching(PrimitiveDenseStore.FACTORY.function().multiply().second(factor), matrix);
